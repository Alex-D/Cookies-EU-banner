...
<p align="center">
    ...
    <br>
    <a href="https://npmx.dev/package/cookies-eu-banner"><img src="https://img.shields.io/npm/v/cookies-eu-banner.svg?color=blue&style=for-the-badge" alt="Version on npm" /></a>
</p>
...
| -               | [Download latest version](https://github.com/Alex-D/cookies-eu-banner/archive/master.zip) |
...
- `#cookies-eu-banner` is the div that contains all elements to be hidden after the user accepts or declines the use of cookies;
...
<script>
	createCookiesBanner({
		bannerElement: document.querySelector("#cookies-eu-banner"),
		onAccept: function () {
...
<script>
	new CookiesEuBanner(function () {
		...
		// Remember to put your own UA-XXXXXXXX-X code
...
The second parameter (`true` in the example below) defines if Cookie EU banner wait the user acceptation before hide the banner. Set to `false` by default.

```html
<script>
	new CookiesEuBanner(function () {
		// Your code to launch when user accepts cookies
	}, true);
</script>
```
...
If you really don't want to save the consent in a cookie, you can use localStorage.

The third parameter (`true` in the example below) defines if Cookie EU banner uses localStorage (`true`) or cookie (`false`). Set to `false` by default.

_Note: the localStorage method is not as good as the cookie method since the localStorage cannot expire after 13 months as recommended._
...
For a detailed explanation, see comments in the main file: [src/index.ts](src/index.ts).
...
3. Shows the banner, then:
   - if the user accepts, run a custom function and put a cookie to save this acceptance;
   - if the user declines, remove all Google Analytics cookies and put a cookie to save this rejection.
